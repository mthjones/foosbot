...
impl CommandHandler for RegisterCommandHandler {
    fn handle(&mut self, sender: &mut Sender, args: &Vec<String>) {
        ...
        if self.registered_users.borrow_mut().insert(username.clone()) {
            sender.respond_in_channel(format!("Added {0} to the daily foosball pool!", username)).unwrap();
        } else {
            sender.respond_in_channel(format!("{0} is already in the daily foosball pool.", username)).unwrap();
        }
    }
}
